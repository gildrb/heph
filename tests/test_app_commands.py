...
def test_model_command_validates_against_session_endpoint(monkeypatch) -> None:
    session = ChatSession(
        config=ChatConfig(api_key="test-key", base_url="https://api.openai.com/v1", model="gpt-5.4"),
        conversation=Conversation(),
        session_id="session-1",
    )
    messages: list[tuple[str, str]] = []

    monkeypatch.setattr(commands.ProviderConfig, "load", classmethod(lambda cls: _default_config()))
    monkeypatch.setattr(commands, "print_success", lambda msg: messages.append(("success", msg)))
    monkeypatch.setattr(commands, "print_error", lambda msg: messages.append(("error", msg)))
