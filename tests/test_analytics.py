...
class TestNoOpWithoutToken:
    ...
    def test_init_is_noop(self, monkeypatch: object) -> None:
        import hephaistos.analytics as mod

        monkeypatch.delenv("POSTHOG_PROJECT_TOKEN", raising=False)  # type: ignore[attr-defined]
    ...
    def test_capture_is_noop(self) -> None:
        ...
        prev = mod._posthog_client
        mod._posthog_client = None
        try:
            capture("test_event", {"key": "value"})  # should not raise
        finally:
            mod._posthog_client = prev
...
class TestInitWithPosthog:
    ...
    def test_init_creates_client(self, monkeypatch: object) -> None:
        import hephaistos.analytics as mod

        monkeypatch.setenv("POSTHOG_PROJECT_TOKEN", "phc_test123")  # type: ignore[attr-defined]
        mod._posthog_client = None

        mock_posthog = MagicMock()
        with patch.dict("sys.modules", {"posthog": mock_posthog}):
            init_analytics()
            assert mod._posthog_client is not None
