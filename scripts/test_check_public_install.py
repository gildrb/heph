...
def test_public_install_builds_versioned_uv_tool_command() -> None:
    command = _uv_tool_install_command("0.1.49", "3.13", DEFAULT_INDEX)

    assert command[-1] == "heph==0.1.49"
...
def test_public_install_derives_expected_runtime_version() -> None:
    assert _expected_runtime_version("0.1.49", None) == "v0.1.49"
    assert _expected_runtime_version("0.1.49", "release-1") == "release-1"
...
def test_public_install_parses_heph_version(monkeypatch, tmp_path) -> None:
    ...
    assert _installed_heph_version(Path("/tmp/heph"), cwd=tmp_path) == "0.1.49"
