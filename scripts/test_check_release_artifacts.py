...
def test_check_release_artifacts_accepts_expected_wheel_and_sdist(tmp_path: Path) -> None:
    dist = tmp_path / "dist"
    dist.mkdir()
    _write_wheel(dist / "heph-0.0.59-py3-none-any.whl")
    _write_sdist(dist / "heph-0.0.59.tar.gz")
...
def test_check_release_artifacts_rejects_internal_workspace_dependency(tmp_path: Path) -> None:
    ...
    dist.mkdir()
    _write_wheel(
        dist / "heph-0.0.59-py3-none-any.whl",
        requires_dist=("harness==0.0.59",),
    )
    _write_sdist(dist / "heph-0.0.59.tar.gz")
...
def _write_wheel(
    ...
    *,
    ...
    metadata_lines = ["Name: heph", "Version: 0.0.59"]
    metadata_lines.extend(f"Requires-Dist: {requirement}" for requirement in requires_dist)
    metadata = "\n".join(metadata_lines) + "\n"
    with zipfile.ZipFile(path, "w") as wheel:
        for member in members:
            wheel.writestr(member, "")
        wheel.writestr("heph-0.0.59.dist-info/METADATA", metadata)
        wheel.writestr(
            "heph-0.0.59.dist-info/entry_points.txt",
...
def _write_sdist(path: Path) -> None:
    with tarfile.open(path, "w:gz") as sdist:
        for suffix in (
            ...
            sdist.add(source, arcname=f"heph-0.0.59/{suffix}")
