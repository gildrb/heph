...
def test_release_build_config_from_env_uses_release_backend_values() -> None:
    ...
    assert 'RELEASE_VERSION: str | None = "v0.0.53"' in rendered
...
def test_clean_dist_removes_release_artifacts_only(tmp_path) -> None:
    (tmp_path / "heph-0.0.53-py3-none-any.whl").write_text("", encoding="utf-8")
    (tmp_path / "heph-0.0.53.tar.gz").write_text("", encoding="utf-8")
