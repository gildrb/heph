...
def test_clean_dist_removes_release_artifacts_only(tmp_path) -> None:
    (tmp_path / "heph-0.0.59-py3-none-any.whl").write_text("", encoding="utf-8")
    (tmp_path / "heph-0.0.59.tar.gz").write_text("", encoding="utf-8")
...
def test_stage_release_project_builds_single_public_package(tmp_path) -> None:
    ...
    assert "harness==0.0.59" not in pyproject
