...
`executable` must be a non-empty string; `max_tokens` and `rag_context_budget`
must be non-negative integers, while `temperature` must be a finite number.
Optional string overrides such as `session_id`, `base_url`, and `model` must be
non-empty when supplied; choice overrides such as `reasoning_level` and
...
validation remains strict against the current advertised contract.
