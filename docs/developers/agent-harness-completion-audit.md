...
| Do not leak fixture-specific courses into runtime harness logic. | `tests/test_native_harness_policy.py` scans `packages/hephaion/` runtime code, and `scripts.audit_agent_harness_completion` also scans non-fixture harness scripts while benchmark fixtures remain free to use realistic examples. | Covered |
...
| German text and LaTeX extraction should not collapse retrieval. | `packages/hephaion/src/hephaion/rag/chunker.py` normalizes extracted text and repairs common umlaut/OCR patterns; `scripts/benchmark_index_integrity.py` gates indexed source text before retrieval; synthetic German/LaTeX fixtures cover regression cases. | Covered for deterministic fixtures; real scanned PDFs still need broader corpus proof |
| User-visible harness events should show reading, retrieval, writing, and acceptance criteria. | `packages/hephaion/src/hephaion/rag/index.py` reports index `reading`, `indexed`, `skipped`, and `writing` progress; `packages/hephaion/src/hephaion/chat/turn_lifecycle.py`, `packages/hephaion/src/hephaion/chat/armory_turn.py`, `packages/hephaion/src/hephaion/chat/evidence_notices.py`, and `packages/hephaion/src/hephaion/chat/turn_finalization.py` emit turn-stream `reading`, material-operation, `evidence`, `writing`, and `verification` notices through the public orchestrator facade; `agent.dispatch` emits `acceptance_criteria` before the first generic tool-verification turn and injects the same criteria into model context; material-operation events carry index counts, search/overview operations, opened excerpts, and read-all scope disclosure; evidence notices carry reviewable refs, source coverage, and excerpt metadata; `heph chat ask --jsonl` exposes those events as JSON Lines for non-interactive harness audits; CLI, TUI streaming, orchestrator facade, and benchmark tests verify these surfaces while calibration evidence stays hidden. | Covered for CLI/chat/TUI stream surfaces |
