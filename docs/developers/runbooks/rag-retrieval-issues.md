...
1. **Verify armory structure** - the armory must have a valid marker:
   ```bash
   cat my-armory/.harness/armory.toml
   ```
...
3. **Check the RAG index** - it should exist and be recent:
   ```bash
   cat my-armory/.harness/rag_index.json | python -m json.tool | head
   ```
...
| Memory not loaded | Check `my-armory/.harness/memory.json` exists and is valid |
