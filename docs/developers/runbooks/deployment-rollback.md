...
The public beta train starts at `v0.0.49`; `v0.0.53` supersedes that first
...
uv run python -m scripts.release_stress_test --expect-runtime-channel pypi --expect-runtime-version v0.0.53
```
...
1. **Yank the release from PyPI** (prevents new installs):
   ```bash
   uvx twine yank heph 0.0.53 --repository pypi
   ```
