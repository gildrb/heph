...
def load_config(armory_path: Path | None = None) -> ChatConfig:
    ...
    runtime = importlib.import_module("hephaion.runtime")
...
def _apply_provider_config(config: ChatConfig) -> None:
    providers_config = importlib.import_module("hephaion.providers.config")
    runtime = importlib.import_module("hephaion.runtime")
    try:
...
_CONFIG_KEY_TO_ENV = {
    "base_url": "HEPHAION_BASE_URL",
    "model": "HEPHAION_MODEL",
    "max_tokens": "HEPHAION_MAX_TOKENS",
    "rag_context_budget": "HEPHAION_RAG_CONTEXT_BUDGET",
    "temperature": "HEPHAION_TEMPERATURE",
    "feature_flags": "HEPHAION_FEATURE_FLAGS",
    "theme": "",
    "default_armory_path": "",
    "activity_trace_mode": "",
...
def _effective_setting_value(key: str) -> str:
    privacy = importlib.import_module("hephaion.privacy.consent")
