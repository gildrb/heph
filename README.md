...
Run the comprehensive external workflow, including optional BEIR, MTEB retrieval,
standard RAG, native, public-academic, and summary phases:
...
`--beir-source-zip` inputs for BEIR runs, local `--mteb-source-dir` or explicit
MTEB JSONL/TSV file inputs for retrieval-only MTEB gates, and
`--require-beir-extra` or `--require-visualization-extra` to fail early when
