...
from harness.documents.priority_analysis import PriorityAnalysis
from harness.documents.priority_progress import (
    _emit_progress,
    _format_elapsed_since,
    _ProgressHeartbeat,
    _write_text_artifact,
)
from harness.documents.priority_rendering import (
    _truncate,
...
from harness.documents.priority_web import (
    duckduckgo_search,
)
from harness.documents.priority_web import (
...
_WEB_PREREQ_ENV = "HARNESS_PRIORITY_WEB_PREREQS"
...
_PRIORITY_SCHEMA = """
{
  ...
  "topics": [
    {
      ...
      "practice_actions": ["concrete, measurable goal grounded in the material"],
  ...
  "document_plan": ["ordered next steps grounded in evidence"],
...
over filename fragments. Make each practice action a concrete, checkable goal rather than a vague
...
def generate_priority_report(
    ...
    *,
    ...
    path = output_dir / f"harness-priority-{datetime.now(UTC):%Y%m%d-%H%M%S}.pdf"
