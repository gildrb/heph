"""Prompt and turn-plan construction for document-backed practice sessions."""

from __future__ import annotations

from dataclasses import dataclass

from extensions.contracts import heph_product_context

from harness.documents.assessment import CONFIDENCE_RE
from harness.documents.policy import DocumentMove, normalize_confidence_value
...
_NO_ENGLISH_RECALL_RULE = (
    "- Do not hard-code an English recall sentence when the practice exchange is in "
    "another language.\n"
)
...
class DocumentTurnPlan:
    ...
    document_move: DocumentMove | None = None
...
def _priority_plan(user_input: str, *, phase: RecallPhase) -> DocumentTurnPlan:
    return _turn_plan(
...
def _chat_prompt_plan(prompt: str, *, phase: RecallPhase) -> DocumentTurnPlan:
    return _turn_plan(DocumentAction.CHAT, prompt, phase=phase)


def _prompt_recall_plan(item: str) -> DocumentTurnPlan:
    return _turn_plan(
...
def _refuse_reveal_plan(item: str, *, phase: RecallPhase) -> DocumentTurnPlan:
    return _turn_plan(DocumentAction.REFUSE_REVEAL, _refusal_prompt(item), phase=phase)
...
def _practice_calibration_prompt(query: str, state: RecallState) -> str:
...
def _refusal_prompt(item: str) -> str:
    return _prompt_frame(
        ...
        rules=(
            ...
            _SAME_LANGUAGE_ITEM_RULE,
            "- Do not hard-code an English refusal when the practice exchange is in another "
            "language.",
            _KEEP_ONE_OR_TWO_SHORT_SENTENCES_RULE,
        ),
    )
...
def _hint_prompt(item: str, hint_level: int) -> str:
    ...
    return _prompt_frame(
        ...
        rules=(
            ...
            _CITE_EVIDENCE_CLAIMS_RULE,
            _SAME_LANGUAGE_ITEM_RULE,
            "- Do not hard-code an English hint when the practice exchange is in another "
            "language.",
            _KEEP_ONE_SHORT_SENTENCE_RULE,
        ),
    )
...
def _open_material_plan_for_intent(user_input: str, intent: str) -> DocumentTurnPlan:
...
def _practice_calibration_plan(state: RecallState, user_input: str) -> DocumentTurnPlan:
    query = _normalize(user_input) or "next material-backed recall item"
    return _turn_plan(
        DocumentAction.CALIBRATE,
...
def _recall_review_plan(state: RecallState, source_query: str) -> DocumentTurnPlan:
...
def _recall_scaffold_plan(state: RecallState, source_query: str) -> DocumentTurnPlan:
    return _turn_plan(
...
def _recall_hint_plan(state: RecallState, source_query: str) -> DocumentTurnPlan:
    return _turn_plan(
        DocumentAction.HINT,
        _hint_prompt(state.current_item, state.hint_level + 1),
        phase=RecallPhase.ASSESS,
        retrieval_query=source_query,
        use_expected_source_refs=True,
    )
