"""Document practice policy primitives.

The policy in this module is intentionally deterministic. It gives the chat
orchestrator a small, inspectable document-backed move instead of relying on a
single large prompt to decide how a practice session should proceed.
"""
...
type DocumentMoveDifficulty = Literal["easy", "medium", "hard"]
type SessionTypeMoveSpec = tuple[DocumentMoveKind, str, DocumentMoveDifficulty | None, str]
type ActionMoveSpec = tuple[DocumentMoveKind, str, str]
...
type DocumentStrategy = Callable[[DocumentPolicyInput, PracticeSessionType], DocumentMove | None]
...
class TurnSummary:
    role: str
    text: str
    feedback: RecallFeedbackType = RecallFeedbackType.NONE
...
@dataclass(frozen=True, slots=True)
class DocumentMove:
    kind: DocumentMoveKind
    reason: str
    requires_evidence: bool
    requires_user_commitment: bool
    difficulty: DocumentMoveDifficulty | None
...
_DRIVEN_LEARNING_INTENTS = frozenset(
    {"driven_recall_calibration", "topic_drill", "priority_request"}
)
_STUDY_INTENTS = frozenset({"driven_recall_calibration", "topic_drill"})
_NEXT_ACTION_BY_FEEDBACK: dict[RecallFeedbackType, DocumentMoveKind] = {
    RecallFeedbackType.CORRECT: "schedule_review",
    RecallFeedbackType.PARTIAL: "give_hint",
    RecallFeedbackType.WRONG: "ask_recall",
}
_CORRECTNESS_BY_FEEDBACK = {
    RecallFeedbackType.CORRECT: 1.0,
    RecallFeedbackType.PARTIAL: 0.55,
    RecallFeedbackType.WRONG: 0.0,
}
...
class DocumentPolicy:
    def next_turn(self, input_data: DocumentPolicyInput) -> DocumentMove:
        if input_data.intent in _DRIVEN_LEARNING_INTENTS:
            return _driven_recall_move(input_data)
        return _material_document_move(input_data)


def is_driven_recall_intent(intent: str) -> bool:
...
def _learner_topic(state: RecallState, requested_topic: str) -> str:
    return requested_topic or state.retrieval_query or state.current_item


def _learner_next_action(state: RecallState) -> DocumentMoveKind:
...
def validate_pedagogy(reply: str, move: DocumentMove) -> PedagogyValidation:
    issues: list[str] = []
...
def _pedagogy_rewrite_instruction(move: DocumentMove) -> str:
    if move.requires_user_commitment:
        return "Rewrite to require an attempt and confidence before revealing more."
    return "Rewrite to match the selected document move without adding unsolicited guidance."


def _looks_like_recall_answer_leak(normalized_reply: str, move: DocumentMove) -> bool:
...
def move_for_plan(
    ...
    *,
    ...
    input_data = DocumentPolicyInput(
    ...
    if move is not None:
        return move
    return DocumentPolicy().next_turn(input_data)
...
def append_policy_prompt(
    ...
    *,
    ...
    if not prompt or action is DocumentAction.CHAT:
        return prompt
...
def _move_policy_lines(move: DocumentMove) -> tuple[str, ...]:
    lines: list[str] = []
    if move.requires_user_commitment:
...
def _action_policy_lines(
    ...
    *,
    ...
    if action is DocumentAction.SOURCE_QA:
        return ("- Source-only answer: do not add tutoring steps after the answer.",)
    if action is DocumentAction.CALIBRATE:
        return _calibration_policy_lines()
    if action is DocumentAction.ASSESS and _needs_contrastive_correction(move):
...
def _needs_contrastive_correction(move: DocumentMove) -> bool:
...
def _direct_answer_move(input_data: DocumentPolicyInput) -> DocumentMove:
    if input_data.recall_state.phase is RecallPhase.RECALL:
...
def _due_review_move(input_data: DocumentPolicyInput) -> DocumentMove | None:
...
def _active_recall_move(state: RecallState) -> DocumentMove | None:
    if state.phase is not RecallPhase.RECALL:
        return None
    if _high_confidence_gap(state):
        return _move(
            "contrastive_question",
...
def _material_document_move(input_data: DocumentPolicyInput) -> DocumentMove:
    for maybe_move in (
        _due_review_move(input_data),
        _active_recall_move(input_data.recall_state),
        _stored_memory_move(input_data),
        _missing_index_move(input_data.material_status),
        _explicit_practice_move(input_data.intent),
    ):
    ...
    return _move(
        "answer",
        "document policy should answer the user's material question",
        requires_evidence=True,
        expected_output_shape="Answer directly from evidence.",
    )


def _missing_index_move(material_status: MaterialStatus) -> DocumentMove | None:
...
def _explicit_practice_move(intent: str) -> DocumentMove | None:
    if intent not in _STUDY_INTENTS:
        return None
    return _document_move(
        "ask_recall",
        "the user asked to practice, so active recall should come before more exposition",
        difficulty="easy",
        expected_output_shape="Ask one material-backed recall question with confidence.",
    )


def _driven_recall_move(input_data: DocumentPolicyInput) -> DocumentMove:
    state = input_data.recall_state
    ...
    for strategy in _PRACTICE_STRATEGIES:
        if move := strategy(input_data, session_type):
            return move
    if state.phase is RecallPhase.RECALL:
        return _material_document_move(input_data)
    return _document_move(
        "ask_recall",
        "practice should drive the next useful active-recall step",
...
def _practice_session_type_move(
    _input_data: DocumentPolicyInput,
    session_type: PracticeSessionType,
) -> DocumentMove | None:
...
_PRACTICE_STRATEGIES: tuple[DocumentStrategy, ...] = (
...
_ACTIVE_RECALL_ACTIONS = frozenset(
    {DocumentAction.CALIBRATE, DocumentAction.PROMPT_RECALL, DocumentAction.SIMPLIFY}
)
...
def _move_from_action(
    ...
    if _is_recall_reprompt(action, input_data.recall_state):
...
def _is_recall_reprompt(action: DocumentAction, state: RecallState) -> bool:
    return action is DocumentAction.PROMPT_RECALL and state.phase is RecallPhase.RECALL


def _recall_reprompt_move(_input_data: DocumentPolicyInput) -> DocumentMove:
    return _move(
        "ask_clarifying_question",
...
def _active_recall_action_move(_input_data: DocumentPolicyInput) -> DocumentMove:
    return _document_move(
        "ask_recall",
...
def _present_action_move(input_data: DocumentPolicyInput) -> DocumentMove:
    if is_driven_recall_intent(input_data.intent):
        return _driven_recall_move(input_data)
...
_ACTION_MOVE_SPECS: dict[DocumentAction, ActionMoveSpec] = {
    DocumentAction.PRIORITY: (
        "diagnose_priority",
        "priority analysis should reduce command burden and choose the next topic",
        "Recommend the next practice action and explain why it helps.",
    ),
    DocumentAction.HINT: (
        "give_hint",
...
_ACTION_MOVE_BUILDERS: dict[DocumentAction, Callable[[DocumentPolicyInput], DocumentMove]] = {
    DocumentAction.ASSESS: _material_document_move,
    DocumentAction.PRESENT: _present_action_move,
}
...
def _move(
    ...
    *,
    ...
    difficulty: DocumentMoveDifficulty | None = None,
...
def _high_confidence_gap(state: RecallState) -> bool:
    return (
        state.last_confidence is not None
        and state.last_confidence >= 0.75
        and state.last_feedback_type in {RecallFeedbackType.PARTIAL, RecallFeedbackType.WRONG}
    )
