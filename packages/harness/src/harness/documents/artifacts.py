"""Source-grounded practice artifact models and validators."""
...
from harness.documents.mastery import next_recall_mastery
...
class DocumentArtifactDifficulty(StrEnum):
    INTRO = "intro"
    CORE = "core"
    ADVANCED = "advanced"
...
class DocumentArtifactSourceSpan:
    source_ref: str
    text: str
    start: int | None = None
    end: int | None = None
...
@dataclass(frozen=True, slots=True)
class DocumentArtifactReviewState:
    reviews: int = 0
    lapses: int = 0
    mastery: float = 0.0
...
class DocumentArtifact:
    ...
    difficulty: DocumentArtifactDifficulty = DocumentArtifactDifficulty.CORE
    source_spans: tuple[DocumentArtifactSourceSpan, ...] = ()
    review_state: DocumentArtifactReviewState = field(default_factory=DocumentArtifactReviewState)


@dataclass(frozen=True, slots=True)
class DocumentArtifactIssue:
    artifact_id: str
    code: str
    message: str
...
@dataclass(frozen=True, slots=True)
class DocumentArtifactValidationResult:
    artifact: DocumentArtifact
    issues: tuple[DocumentArtifactIssue, ...] = ()
...
@dataclass(frozen=True, slots=True)
class DocumentArtifactValidationReport:
    results: tuple[DocumentArtifactValidationResult, ...]

    @property
    def accepted(self) -> tuple[DocumentArtifact, ...]:
        return tuple(result.artifact for result in self.results if result.accepted)

    @property
    def rejected(self) -> tuple[DocumentArtifactValidationResult, ...]:
...
_INVALID_DIFFICULTY_MESSAGE = "difficulty must be a DocumentArtifactDifficulty"
_MAX_ARTIFACT_REVIEW_INTERVAL_DAYS = 365
type _ArtifactRule = tuple[str, str, Callable[[DocumentArtifact], bool]]


def _valid_flashcard(artifact: DocumentArtifact) -> bool:
    return bool(_clean(artifact.prompt) and _clean(artifact.answer))


def _valid_cloze_card(artifact: DocumentArtifact) -> bool:
    return bool(_CLOZE_RE.search(_clean(artifact.content) or _clean(artifact.prompt)))


def _valid_quiz(artifact: DocumentArtifact) -> bool:
    return "?" in _clean(artifact.prompt) and bool(_clean(artifact.answer))


def _valid_summary(artifact: DocumentArtifact) -> bool:
    return len(_TOKEN_RE.findall(_clean(artifact.content) or _clean(artifact.answer))) >= 8


def _valid_misconception(artifact: DocumentArtifact) -> bool:
    return bool(_clean(artifact.content) or _clean(artifact.answer))


def _valid_concept_tag(artifact: DocumentArtifact) -> bool:
    return bool(artifact.concept_tags)


def _valid_prerequisite(artifact: DocumentArtifact) -> bool:
    return bool(artifact.prerequisite_tags)


_ARTIFACT_KIND_RULES: dict[DocumentArtifactKind, _ArtifactRule] = {
    DocumentArtifactKind.FLASHCARD: (
        "invalid_flashcard",
        "flashcards need prompt and answer",
        _valid_flashcard,
    ),
    DocumentArtifactKind.CLOZE_CARD: (
        "invalid_cloze",
        "cloze cards need {{c1::...}} text",
        _valid_cloze_card,
    ),
    DocumentArtifactKind.QUIZ: (
        "invalid_quiz",
        "quizzes need a question and answer",
        _valid_quiz,
    ),
    DocumentArtifactKind.SUMMARY: (
        "invalid_summary",
        "summaries need supported content",
        _valid_summary,
    ),
    DocumentArtifactKind.MISCONCEPTION: (
        "invalid_misconception",
        "misconceptions need a correction",
        _valid_misconception,
    ),
    DocumentArtifactKind.CONCEPT_TAG: (
        "invalid_concept_tag",
        "concept tags are required",
        _valid_concept_tag,
    ),
    DocumentArtifactKind.PREREQUISITE: (
...
def validate_document_artifact(
    ...
    return DocumentArtifactValidationResult(artifact, _artifact_issues(artifact, source_map))


def validate_document_artifacts(
    artifacts: Sequence[DocumentArtifact],
    source_text_by_ref: Mapping[str, str] | None = None,
) -> DocumentArtifactValidationReport:
    seen: dict[str, str] = {}
    results: list[DocumentArtifactValidationResult] = []
    source_map = source_text_by_ref or {}
    for artifact in artifacts:
        ...
        results.append(DocumentArtifactValidationResult(artifact=artifact, issues=tuple(issues)))
    return DocumentArtifactValidationReport(results=tuple(results))


def document_artifacts_to_anki_tsv(
    artifacts: Sequence[DocumentArtifact],
    source_text_by_ref: Mapping[str, str] | None = None,
) -> str:
    report = validate_document_artifacts(artifacts, source_text_by_ref)
    if report.rejected:
...
def _rejected_artifact_export_message(
    rejected: Sequence[DocumentArtifactValidationResult],
) -> str:
    failures = [
        f"{result.artifact.artifact_id}: {', '.join(issue.code for issue in result.issues)}"
        for result in rejected
    ]
    return "cannot export rejected practice artifacts: " + "; ".join(failures)


def _anki_row_for_artifact(artifact: DocumentArtifact) -> tuple[str, str, str, str, str] | None:
...
def _anki_front_back(artifact: DocumentArtifact) -> tuple[str, str] | None:
    if artifact.kind is DocumentArtifactKind.CLOZE_CARD:
        return artifact.content or artifact.prompt, artifact.answer
    if artifact.kind in {DocumentArtifactKind.FLASHCARD, DocumentArtifactKind.QUIZ}:
...
def next_document_artifact_review_state(
    state: DocumentArtifactReviewState,
    rating: RecallRating,
    *,
    ...
    return DocumentArtifactReviewState(
        reviews=reviews,
        lapses=lapses,
        mastery=mastery,
...
def record_document_artifact_review(
    artifact: DocumentArtifact,
    rating: RecallRating,
    *,
    reviewed_at: datetime,
    ...
    return replace(
        artifact,
        review_state=next_document_artifact_review_state(
...
) -> tuple[DocumentArtifactIssue, ...]:
    issues: list[DocumentArtifactIssue] = []
...
def _shape_issues(artifact: DocumentArtifact) -> tuple[DocumentArtifactIssue, ...]:
    issues: list[DocumentArtifactIssue] = []
    ...
    if isinstance(artifact.kind, DocumentArtifactKind) and (
...
def _enum_shape_issues(artifact: DocumentArtifact) -> tuple[DocumentArtifactIssue, ...]:
    issues: list[DocumentArtifactIssue] = []
    if not isinstance(artifact.kind, DocumentArtifactKind):
        issues.append(_issue(artifact, "invalid_kind", "kind must be a DocumentArtifactKind"))
    if not isinstance(artifact.difficulty, DocumentArtifactDifficulty):
        issues.append(_issue(artifact, "invalid_difficulty", _INVALID_DIFFICULTY_MESSAGE))
    return tuple(issues)


def _kind_rule_issue(artifact: DocumentArtifact) -> DocumentArtifactIssue | None:
...
def _single_source_span_issues(
    artifact: DocumentArtifact,
    span: DocumentArtifactSourceSpan,
    source_text_by_ref: Mapping[str, str],
) -> tuple[DocumentArtifactIssue, ...]:
...
def _has_empty_source_span_part(span: DocumentArtifactSourceSpan) -> bool:
...
def _missing_source_span_message(span: DocumentArtifactSourceSpan) -> str:
...
def _span_has_offsets(span: DocumentArtifactSourceSpan) -> bool:
    return span.start is not None and span.end is not None


def _span_has_unpaired_offset(span: DocumentArtifactSourceSpan) -> bool:
    return (span.start is None) != (span.end is None)


def _span_offsets_are_invalid(span: DocumentArtifactSourceSpan) -> bool:
...
def _review_state_issues(artifact: DocumentArtifact) -> tuple[DocumentArtifactIssue, ...]:
...
def _span_matches_source(span: DocumentArtifactSourceSpan, source_text: str) -> bool:
...
def _span_offsets_select_source(span: DocumentArtifactSourceSpan, source_text: str) -> bool:
    return (
...
def _artifact_fingerprint(artifact: DocumentArtifact) -> str:
    kind = artifact.kind
    kind_value = kind.value if isinstance(kind, DocumentArtifactKind) else str(kind)
    parts = (
        kind_value,
...
def _anki_tags(artifact: DocumentArtifact) -> tuple[str, ...]:
    source_tags = tuple(
...
def _issue(artifact: DocumentArtifact, code: str, message: str) -> DocumentArtifactIssue:
    return DocumentArtifactIssue(artifact_id=artifact.artifact_id, code=code, message=message)
