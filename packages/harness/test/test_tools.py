...
from harness.agent.web_tools import (
...
class TestRunBash:
    ...
    def test_rtk_enabled_uses_ultra_compact_flag(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {"HARNESS_RTK": "1", "HARNESS_RTK_ULTRA": "1"},
                clear=False,
            ),
            patch("harness.agent.shell_tools.shutil.which", return_value="/usr/local/bin/rtk"),
            patch("harness.agent.shell_tools.subprocess.run", return_value=completed) as run,
    ...
    def test_rtk_min_command_chars_skips_short_commands(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {"HARNESS_RTK": "1", "HARNESS_RTK_MIN_COMMAND_CHARS": "999"},
                clear=False,
            ),
...
class TestArmoryTools:
    def test_create_armory_builds_canonical_layout(self, tmp_path: Path) -> None:
        ...
        assert (tmp_path / "linear-algebra" / ".harness" / "armory.toml").is_file()
        assert (tmp_path / "linear-algebra" / ".harness" / "chats").is_dir()
    ...
    def test_create_named_armory_uses_exact_home_name(self, tmp_path: Path) -> None:
        ...
        with patch.dict("os.environ", {"HARNESS_ARMORY_HOME": str(home)}, clear=False):
            result = run_create_named_armory("bfi-2", workspace=tmp_path)

        assert result.success is True
        assert result.metadata["created"] is True
        assert (home / "bfi-2" / "materials").is_dir()
        assert (home / "bfi-2" / ".harness" / "armory.toml").is_file()
    ...
    def test_memory_tool_adds_and_reads_armory_memory(self, tmp_path: Path) -> None:
        ...
        assert (tmp_path / ".harness" / "memory.json").is_file()
...
class TestWorkspaceFileTools:
    ...
    def test_write_file_denies_other_armory_state_files(self, tmp_path: Path) -> None:
        ...
        assert result == "Access denied: .harness contains internal armory state."
        assert not (tmp_path / ".harness" / "memory.json").exists()
...
class TestWebFetch:
    ...
    def test_rejects_url_credentials(self):
        with patch("harness.agent.web_tools._open_without_redirect") as open_url:
    ...
    def test_fetch_includes_source_attribution(self):
        ...
        with patch(
            "harness.agent.web_tools._open_without_redirect",
    ...
    def test_fetch_https_request_preserves_original_host_identity(self):
        ...
        with (
            patch(
                "harness.agent.web_tools._resolve_hostname_ips",
    ...
    def test_pinned_https_connection_uses_validated_ip_and_original_sni(self):
        ...
        with patch(
            "harness.agent.web_tools.socket.create_connection",
            return_value=socket,
        ) as create:
            connection.connect()
    ...
    def test_fetch_blocks_private_redirect_target(self):
        ...
        with (
            patch("harness.agent.web_tools._resolve_hostname_ips", side_effect=resolve),
            patch("harness.agent.web_tools._open_without_redirect", side_effect=redirect),
        ):
    ...
    def test_fetch_blocks_non_global_carrier_grade_nat_target(self):
        with (
            ...
            patch("harness.agent.web_tools._open_without_redirect") as open_url,
        ):
