...
def test_external_latex_compiler_disables_shell_escape(
    tmp_path: Path,
    ...
    monkeypatch.setattr("harness.documents.priority.subprocess.run", fake_run)
    ...
    assert commands
    assert all("-no-shell-escape" in command for command in commands)


def test_tectonic_compiler_uses_cached_packages_only(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tex_path = tmp_path / "report.tex"
...
def test_priority_report_writes_printable_pdf_latex_from_local_evidence(tmp_path: Path) -> None:
    ...
    assert "HARNESS PRIORITY" not in tex
