...
from harness.chat.turn_outputs import _DocumentAgentOutput
from harness.documents.policy import EvidenceAssessment
from harness.documents.prompt_plans import DocumentTurnPlan
from harness.documents.state import DocumentAction, RecallPhase
...
def test_structural_relevance_guard_replaces_reply_before_emit(tmp_path: Path) -> None:
    ...
    events = list(
        orchestrator._iter_agent_document_reply_events(
...
def test_structural_relevance_guard_keeps_recall_state_on_rewrite(
    tmp_path: Path,
) -> None:
    session = ChatSession(
        config=ChatConfig(),
