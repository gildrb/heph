...
class TestLoadOrBuild:
    ...
    def test_rebuilds_poisoned_legacy_cache_even_when_file_hashes_match(
        ...
        (armory / ".harness" / "rag_index.json").write_text(
            json.dumps(
                {
                    "version": 6,
                    "chunk_size": 500,
    ...
    def test_rebuilds_failed_pdf_index_when_conversion_unavailable(
        ...
        monkeypatch.setattr("harness.rag.index._can_convert_binary_file", lambda _path: False)
        monkeypatch.setattr("harness.rag.index.chunk_file", lambda *_args, **_kwargs: None)
        arm = tmp_path / "armory"
        (arm / "materials").mkdir(parents=True)
        ...
        (arm / ".harness" / "rag_index.json").write_text(
            json.dumps(
                {
                    "version": 6,
                    "chunk_size": 500,
    ...
    def test_rebuilds_failed_pdf_index_when_conversion_becomes_available(
        ...
        monkeypatch.setattr("harness.rag.index.chunk_file", fake_chunk_file)
...
class TestScanUnindexableFiles:
    ...
    def test_respects_armory_ignore_patterns(
        ...
        (arm / ".harness").mkdir(parents=True)
        (arm / ".harnessignore").write_text("materials/private/\n", encoding="utf-8")
