...
import pytest
from harness.diagnostics.events import capture, get_distinct_id, init_analytics
from harness.privacy import consent
...
def test_capture_is_noop_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("harness.diagnostics.events.analytics_backend_available", lambda: False)
...
def test_capture_posts_sanitized_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("harness.diagnostics.events.analytics_backend_available", lambda: True)
    monkeypatch.setattr("harness.diagnostics.events.analytics_enabled", lambda: True)
    monkeypatch.setattr("harness.diagnostics.events.posthog_project_token", lambda: "phc_test")
    monkeypatch.setattr(
        "harness.diagnostics.events.posthog_host", lambda: "https://app.posthog.com"
    )
    monkeypatch.setattr("harness.diagnostics.events.get_distinct_id", lambda: "heph_test")
    monkeypatch.setattr(
        "harness.diagnostics.events.runtime_context",
        lambda: {"app_version": "0.0.55", "release_channel": "pypi"},
    )
    monkeypatch.setattr("harness.diagnostics.events.urllib.request.urlopen", _fake_urlopen)
...
def test_capture_rejects_non_https_posthog_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("harness.diagnostics.events.analytics_enabled", lambda: True)
    monkeypatch.setattr("harness.diagnostics.events.posthog_project_token", lambda: "phc_test")
    monkeypatch.setattr("harness.diagnostics.events.posthog_host", lambda: "http://example.com")
    monkeypatch.setattr("harness.diagnostics.events.urllib.request.urlopen", _fake_urlopen)
...
def test_init_analytics_is_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("harness.diagnostics.events.analytics_backend_available", lambda: True)
    monkeypatch.setattr(
        "harness.diagnostics.events.install_id",
