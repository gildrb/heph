...
from harness.memory.extract import extract_and_store, extract_from_exchange
...
class TestExtractFromExchange:
    def test_short_content_returns_empty(self) -> None:
        ...
        with patch("harness.memory.extract.build_client") as mock_build:
    ...
    def test_extraction_model_env_overrides_chat_model(
        ...
        config = _make_config()
        monkeypatch.setenv("HARNESS_EXTRACTION_MODEL", "cheap-extractor")
        mock_response = _mock_llm_response("[]")
    ...
    def test_codex_subscription_uses_runtime_stream_path(self) -> None:
        ...
        with (
            patch("harness.memory.extract.build_client") as mock_build,
            patch(
                "harness.memory.extract.stream_reply",
                return_value=iter(
                    [
                        (
...
class TestExtractAndStore:
    def test_stores_entries_and_returns_count(self, tmp_path: Path) -> None:
        config = _make_config()
    ...
    def test_returns_zero_when_nothing_extracted(self, tmp_path: Path) -> None:
        config = _make_config()
        memory = MemoryStore(tmp_path)
    ...
    def test_does_not_save_when_no_entries(self, tmp_path: Path) -> None:
        config = _make_config()
        memory = MemoryStore(tmp_path)
        mock_response = _mock_llm_response("[]")
        ...
        with patch("harness.memory.extract.build_client") as mock_build:
            ...
            with patch("harness.memory.extract.save_memory") as mock_save:
                added = extract_and_store(config, memory, "q", "a" * 200)

        assert added == 0
        mock_save.assert_not_called()

    def test_saves_memory_when_entries_added(self, tmp_path: Path) -> None:
        config = _make_config()
        memory = MemoryStore(tmp_path)
