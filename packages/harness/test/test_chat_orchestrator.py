...
import harness.chat.evidence as evidence_module
import pytest
...
from harness.chat.model_text import _model_json_payload
...
from harness.chat.turn_contract_checks import _plan_requires_citations
...
def test_source_qa_repair_uses_compact_model_repair_when_reply_has_no_citations() -> None:
    ...
    with patch(
        "harness.chat.model_text.stream_completion",
        return_value=iter(
...
def test_classified_user_intent_normalizes_model_json_payload(
    ...
    with patch("harness.chat.model_text._model_json_payload", return_value=payload) as model_json:
...
def test_model_json_payload_parses_streamed_json_fragment() -> None:
    ...
    with patch(
        "harness.chat.model_text._stream_one_shot_model_text",
...
def test_expand_prior_evidence_merges_prior_refs_with_query_results() -> None:
    ...
    with (
        patch("harness.chat.evidence.ensure_rag_index", return_value=index),
        patch(
            "harness.chat.evidence._retrieve_query_scored_chunks",
...
def test_turn_evidence_filters_low_content_chunks_from_refs_and_overview() -> None:
    ...
    with patch("harness.chat.evidence.ensure_rag_index", return_value=index):
...
def test_material_overview_followup_reuses_prior_overview_refs_before_query_expansion() -> None:
    ...
    with (
        patch("harness.chat.evidence.ensure_rag_index", return_value=index),
        patch("harness.chat.evidence._retrieve_query_scored_chunks") as retrieve_query,
    ):
...
def test_specific_present_turn_with_overview_strategy_uses_query_evidence() -> None:
    ...
    with (
        patch(
            "harness.chat.evidence.build_turn_evidence_from_query",
            return_value=query_evidence,
        ) as query,
        patch(
            "harness.chat.evidence.build_turn_evidence_from_overview",
...
def test_overview_fallback_reply_uses_model_repair_with_citations() -> None:
    ...
    with patch("harness.chat.model_text._stream_one_shot_model_text", return_value=model_reply):
...
def test_overview_fallback_uses_deterministic_reply_when_model_repair_fails() -> None:
    ...
    with patch("harness.chat.model_text._model_json_payload", return_value={"answer": "uncited"}):
        reply = _overview_fallback_reply(
            plan,
            evidence,
            user_input="overview",
...
def test_overview_fallback_skips_title_metadata_for_substantive_cues() -> None:
    ...
    with patch("harness.chat.model_text._stream_one_shot_model_text", return_value=""):
        reply = _overview_fallback_reply(
            plan,
            evidence,
...
def test_plain_orchestrator_does_not_classify_without_armory() -> None:
    ...
    with (
        patch("harness.chat.intent_resolution._classified_user_intent") as classify,
        patch(
            "harness.chat.turn_execution.stream_completion",
