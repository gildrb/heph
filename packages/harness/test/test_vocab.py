...
import pytest
from harness.parameters.settings import VOCAB_STRICTNESS_LENIENT, AppSettings
from harness.vocab import drill
from harness.vocab.parser import (
    VocabCard,
    parse_vocab_file,
    scan_armory,
)
from harness.vocab.scheduler import (
    Rating,
    schedule_card,
    select_due_cards,
)
from harness.vocab.state import (
    VocabCardState,
    VocabScheduleStore,
    load_schedule,
)
...
class TestDrillIntegration:
    ...
    def test_json_file_structure(self, tmp_path: Path) -> None:
        ...
        data = json.loads((arm / ".harness" / "vocab_schedule.json").read_text())
