"""Learning trace payloads, review signals, and policy intervention helpers."""
...
from study.state import LearningAction, LearningFeedbackType

from chat.turn_predicates import _overview_turn

if TYPE_CHECKING:
    from study.policy import LearningMoveKind
    from study.prompt_plans import LearningTurnPlan
    from study.schedule import RecallItemState, RecallScheduleStore
...
def _trace_turn_retrieval_query(resolved: ResolvedTurnPlan) -> str:
    ...
    if resolved.learning_plan is None or resolved.learning_plan.retrieval_query is None:
        return ""
    return resolved.learning_plan.retrieval_query
