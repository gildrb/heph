...
from heph_interfaces.terminal import current_theme_name, set_theme
from heph_interfaces.tui import armory as tui_armory
from heph_interfaces.tui import keymap
from heph_interfaces.tui import transcript as tui_transcript
from heph_interfaces.tui.armory_browser import armory_detail, build_entries, default_armory_home
...
def test_shift_tab_opens_reasoning_level_control(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.composer_controls.prefetch_provider_model_catalogs",
...
def test_footer_hints_show_idle_shortcuts(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("heph_interfaces.tui.display_text.armory_shortcut_key", lambda: "ctrl+a")
...
def test_status_sidebar_and_footer_chrome_labels_share_one_token(
    ...
    monkeypatch.setattr("heph_interfaces.tui.display_text.armory_shortcut_key", lambda: "ctrl+o")
    session = _plain_session()
...
def test_tui_config_error_allows_openai_codex_oauth(
    ...
    monkeypatch.setattr(
        "heph_ai.runtime.engine.load_credentials",
...
def test_armory_browser_entries_include_recent_and_missing_armories(
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.armory_browser.load_known_armory_entries",
        lambda: [
...
def test_models_inline_menu_aligns_descriptions(
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.model_flows.configured_model_choices",
        fake_model_choices,
    )
    app = tui.HephTui(
        _keyless_session(),
...
def test_logout_inline_menu_lists_only_clearable_stored_credentials(
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.auth_flows.list_providers",
        lambda: ["openai-codex"],
    )
    ...
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.retrieve_key", fake_retrieve_key)
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.get_volatile", fake_get_volatile)
...
def test_logout_inline_names_environment_credentials_when_none_clearable(
    ...
    monkeypatch.setattr(ProviderConfig, "load", classmethod(lambda _cls: default_config()))
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.list_providers", list)
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.retrieve_key", lambda _slug: None)
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.get_volatile", lambda _slug: None)
...
def test_logout_inline_clears_selected_credential_kind_for_duplicate_slug(
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.auth_flows.retrieve_key",
    ...
    monkeypatch.setattr(
        "heph_interfaces.tui.auth_flows.clear_credentials",
        cleared_oauth.append,
    )
    monkeypatch.setattr("heph_interfaces.tui.auth_flows.clear_key", cleared_keys.append)
...
def test_external_command_collects_plain_output_for_compact_append(
    ...
    monkeypatch.setattr("heph_interfaces.terminal.input.handle_input", fake_handle_input)
